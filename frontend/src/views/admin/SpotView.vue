<template>
  <div class="admin-page">
    <el-card>
      <template #header>
        <div class="card-header">
          <span><el-icon><Place /></el-icon> 景点管理</span>
          <el-button type="primary" @click="openForm(null)"><el-icon><Plus /></el-icon> 新增景点</el-button>
        </div>
      </template>
      <div class="search-bar">
        <el-input v-model="keyword" placeholder="搜索景点名称" clearable style="width:200px" @clear="handleSearch" @keydown.enter="handleSearch" />
    ...
    <el-dialog v-model="formVisible" :title="formData.id ? '编辑景点' : '新增景点'" width="600px">
      <el-form :model="formData" label-width="100px">
        ...
        <el-form-item label="分类"><el-input v-model="formData.category" placeholder="自然/人文/美食..." /></el-form-item>
      </el-form>
      <template #footer>
...
const formData = reactive({ cityId: 1, name: "", description: "", address: "", imageUrl: "", rating: 0, price: 0, category: "" })

onMounted(() => loadData())
async function loadData() {
  loading.value = true
  try {
    const res = await spotApi.page({ pageNum: pageNum.value, pageSize: pageSize.value, keyword: keyword.value || undefined })
...
function openForm(row: any) {
  ...
  else Object.assign(formData, { id: undefined, cityId: 1, name: "", description: "", address: "", imageUrl: "", rating: 0, price: 0, category: "" })
  formVisible.value = true
}
async function handleSave() {
  saving.value = true
  try {
    if (formData.id) { await spotApi.update(formData.id, formData); ElMessage.success("更新成功") }
    else { await spotApi.create(formData); ElMessage.success("创建成功") }
    formVisible.value = false; await loadData()
  } finally { saving.value = false }
}
async function handleDelete(id: number) {
  try { await ElMessageBox.confirm("确定删除？", "提示"); await spotApi.delete(id); ElMessage.success("删除成功"); await loadData() } catch { }
}
</script>
<style scoped>
